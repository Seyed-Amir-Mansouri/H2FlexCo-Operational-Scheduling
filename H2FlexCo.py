...
def load_data(file_path):
    ...
    Returns:
        tuple: Various data structures including hours, electrolysers, prices, capacities, etc.
    """
...
def create_model(hours, electrolysers, price_dam, pv, wind, wind_cap, pv_cap, ees_cap, el_max, el_min, comp, eff, flex):
    """
    Create and configure the Pyomo optimization model.
    
    Args:
        Various data structures from load_data function
    ...
    # Sets
    model.e = RangeSet(1, 5)  # Electrolysis units
    model.t = RangeSet(1, 24)  # Time periods (24 hours)

    # Parameters
    model.lambda_DAM_Buy = Param(model.t, initialize=price_dam) 
    model.lambda_DAM_Sell = Param(model.t, initialize=lambda model, t: 0.9 * price_dam[t]) 
    ...
    model.alpha_Green = Param(initialize=0.9)  
...
def add_constraints(model):
    ...
    Args:
        model: Pyomo model to add constraints to
    """
    # Constraint definitions
    def constraint_d2(model, t):
        return model.c_CfD[t] == (model.lambda_VPPA - model.lambda_DAM_Buy[t]) * model.P_VPPA_H2FlexCo[t]
    ...
    def constraint_d5(model, e, t):
        return model.p_Local[e, t] == model.F_Wind[t] * model.Wind_Max[e] + model.F_PV[t] * model.PV_Max[e] 
    ...
    def constraint_d17(model, e, t):
        return model.i_EES_Ch[e, t] + model.i_EES_Dis[e, t] <= 0
    ...
    def constraint_d28(model):
        ...
        - model.n_GO_Sell[e, t] for e in model.e for t in model.t) >= model.alpha_Green * sum((model.h_BD[e, t] + model.DSP[e, t] + model.h_UBD[e, t]) / model.eta_EL[e] 
...
def add_objective(model):
    ...
    Args:
        model: Pyomo model to add objective to
    """
    model.z_H2FlexCo = Objective(
        expr=sum(
            (model.lambda_BD * model.h_BD_H2FlexCo[t] + model.lambda_UBD * model.h_UBD_H2FlexCo[t])
            + (model.lambda_GO_Sell * model.n_Sell_H2FlexCo[t] - model.lambda_GO_Buy * model.n_Buy_H2FlexCo[t])
            + (model.lambda_DAM_Sell[t] * model.p_DAM_H2FlexCo_Sell[t] - model.lambda_DAM_Buy[t] * model.p_DAM_H2FlexCo_Buy[t])
            - ((1 + model.alpha_Loss) * model.lambda_PPPA * model.P_PPPA_H2FlexCo[t] + model.lambda_GA_TR * model.P_PPPA_H2FlexCo[t])
            - model.c_CfD[t]
            for t in model.t
        ),
        sense=maximize
...
def plot_input_data(hours, wind, pv, price_dam, model):
    """
    Plot input data including renewable generation and electricity prices.
    
    Args:
        hours: List of hours
        wind: Wind generation data
        pv: PV generation data
        price_dam: Day-ahead market prices
        model: Pyomo model
    """
...
def solve_model(model):
    """
    Solve the optimization model and print results.
    
    Args:
        model: Pyomo model to solve
        
    Returns:
        SolverResults: Results from the solver
    """
    solver = SolverFactory('glpk')
    result = solver.solve(model)
    
    print(result.solver.termination_condition)
    print("Objective value:", round(value(model.z_H2FlexCo),3), "€")
    print("-------------------------------------------------------------------------------------------------------------") 
...
def plot_output_data(hours, electrolysers, model):
    """
    Plot output data including power balance, hydrogen balance, and GO balance.
    
    Args:
        hours: List of hours
        electrolysers: List of electrolysers
        model: Pyomo model with solved results
    """
    sns.set_context("notebook")
    sns.set_style("whitegrid")
...
def plot_detailed_results(hours, electrolysers, model):
    """
    Plot detailed results for each electrolyzer.
    ...
    for col_idx, e in enumerate(electrolysers):
        # Power balance for each electrolyzer
        ...
        # Hydrogen balance for each electrolyzer
        df_h2 = pd.DataFrame({
            "Time": np.array(hours),
        ...
        # GO balance for each electrolyzer
        df_go = pd.DataFrame({
            "Time": np.array(hours),
...
def export_results(model):
    ...
    lambda_H2FlexCo = {(1, t): model.lambda_DAM_Buy[t] for t in model.t}
    ...
    data=[P_H2FlexCo_buy, P_H2FlexCo_sell, lambda_H2FlexCo_buy, lambda_H2FlexCo_sell, N_H2FlexCo_buy, N_H2FlexCo_sell, lambda_GO_buy, lambda_GO_sell, P_PPA, lambda_PPA]
    
    with open("H2FlexCo.pkl", "wb") as f:
        pickle.dump(data, f)
    
    print("Results exported to H2FlexCo.pkl")


def main():
    """
    Main function to execute the H2FlexCo optimization model.
    """
    print("H2FlexCo Optimization Model")
    print("=" * 50)
    ...
    model = create_model(hours, electrolysers, price_dam, pv, wind, wind_cap, pv_cap, ees_cap, el_max, el_min, comp, eff, flex)
    ...
    plot_input_data(hours, wind, pv, price_dam, model)
    ...
    # Plot output data
    print("Plotting output data...")
    plot_output_data(hours, electrolysers, model)
    
    # Plot detailed results
    print("Plotting detailed results...")
    plot_detailed_results(hours, electrolysers, model)
    
    # Export results
    print("Exporting results...")
    export_results(model)
    
    print("H2FlexCo optimization completed successfully!")
